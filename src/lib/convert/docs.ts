...
export async function docxConvert(
	files: File[],
	...
	for (let i = 0; i < files.length; i++) {
		const f = files[i];
		progress({ ratio: i / files.length, label: `reading ${f.name} (${i + 1}/${files.length})` });
		...
		if (target === 'markdown') {
			...
			out = new TurndownService({ headingStyle: 'atx', codeBlockStyle: 'fenced' }).turndown(html);
			ext = 'md';
			mime = 'text/markdown';
		} else if (target === 'text') {
		...
		results.push({ name: replaceExt(f.name, ext), blob: new Blob([out], { type: mime }), from: f.name });
	}
	progress({ ratio: 1 });
	return results;
}
		...
		for (const sheetName of wb.SheetNames) {
			...
			const name =
				wb.SheetNames.length > 1
					? `${base}-${sheetName.replace(/[^\w-]+/g, '_')}.csv`
					: `${base}.csv`;
			results.push({ name, blob: new Blob([csv], { type: 'text/csv' }), from: f.name });
		}
	}
	progress({ ratio: 1 });
	return results;
