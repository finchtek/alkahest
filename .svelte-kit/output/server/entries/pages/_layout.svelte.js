...
function Header($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		Logo($$renderer, {});
		$$renderer.push(`<!----> <span class="flex flex-col leading-none"><span>${escape_html(SITE.name)}</span> <span class="mt-0.5 text-[10px] font-medium tracking-normal text-zinc-600">${escape_html(SITE.byline)}</span></span></a> <nav class="flex items-center gap-1 text-sm sm:gap-2"><a href="/#tools" class="rounded-lg px-3 py-1.5 text-zinc-700 transition hover:bg-zinc-100 hover:text-zinc-900">tools</a> <a${attr("href", SITE.tipUrl)} target="_blank" rel="noopener noreferrer" class="hidden items-center gap-1.5 rounded-lg border border-[#a34a32]/40 bg-[#a34a32]/10 px-3 py-1.5 font-medium text-[#a34a32] transition hover:bg-[#a34a32]/20 sm:inline-flex"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 21s-7.5-4.7-10-9.3C.4 8.6 2.2 5 5.7 5c2 0 3.4 1.1 4.3 2.4h4c.9-1.3 2.3-2.4 4.3-2.4 3.5 0 5.3 3.6 3.7 6.7C19.5 16.3 12 21 12 21Z"></path></svg> tip jar</a></nav></div></header>`);
	});
}
//#endregion
...
function Footer($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$$renderer.push(`<!----> <span>${escape_html(SITE.name)} <span class="text-xs font-medium text-zinc-600">${escape_html(SITE.byline)}</span></span></div> <p class="mt-3 text-sm leading-relaxed text-zinc-700">all processing happens locally on your device using WebAssembly. your files are never
					uploaded anywhere or tracked. promise.</p></div> <nav class="grid grid-cols-2 gap-x-12 gap-y-2 text-sm" aria-label="Footer"><a href="/#tools" class="text-zinc-700 transition hover:text-zinc-900">all tools</a> <a${attr("href", SITE.tipUrl)} target="_blank" rel="noopener noreferrer" class="text-zinc-700 transition hover:text-zinc-900">tip jar</a> <a href="/legal" class="text-zinc-700 transition hover:text-zinc-900">terms &amp; privacy</a></nav></div> <p class="mt-8 text-xs text-zinc-600">no cookies · no analytics · no servers · built with SvelteKit, FFmpeg (WASM), pdf-lib, pdf.js,
